...
const UserModel = {
  ...
  update: async (id, { nom, email, role, passwordHash }) => {
    ...
    const values = [String(nom).trim().substring(0, 100), String(email).trim().toLowerCase().substring(0, 150), role === "admin" ? "admin" : "user"];
    if (passwordHash) {
...
module.exports = { UserModel, AgenceModel, BoiteModel, ArchiveModel };
