...
const registerSchema = (sequelize) => {
  ...
  const AgenceTypeDocument = sequelize.define(
    "AgenceTypeDocument",
    {
      agence_id: {
  ...
  const MetaFieldValue = sequelize.define(
    "MetaFieldValue",
    {
      id: { type: DataTypes.INTEGER, primaryKey: true, autoIncrement: true },
      agence_id: { type: DataTypes.INTEGER, allowNull: false },
  ...
  Archive.belongsTo(TypeDocument, {
    foreignKey: "type_document_id",
    onDelete: "SET NULL",
  });
  Agence.belongsToMany(TypeDocument, {
    through: AgenceTypeDocument,
    foreignKey: "agence_id",
  });
  TypeDocument.belongsToMany(Agence, {
    through: AgenceTypeDocument,
    foreignKey: "type_document_id",
  });
  MetaField.belongsTo(TypeDocument, {
    foreignKey: "type_document_id",
    onDelete: "CASCADE",
  });
  MetaFieldValue.belongsTo(Agence, {
    foreignKey: "agence_id",
    onDelete: "CASCADE",
  });
  MetaFieldValue.belongsTo(TypeDocument, {
    foreignKey: "type_document_id",
    onDelete: "CASCADE",
  });
  MetaFieldValue.belongsTo(MetaField, {
